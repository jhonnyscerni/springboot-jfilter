package br.com.siberius.jfilter.jpa.impl;
...
import br.com.siberius.jfilter.core.TriFunction;
...
public class JFilterRepositoryImpl<T> implements JFilterRepository<T> {
	...
	private Map<Aggregable.Operation, TriFunction<String, Root<T>, CriteriaBuilder, Expression<? extends Number>>> operations = new HashMap<>();
	...
	@PostConstruct
	protected void init() {
		operations.put(Aggregable.Operation.COUNT, (dataField, root, cb) -> cb.count(root.get(dataField)));
		operations.put(Aggregable.Operation.SUM, (dataField, root, cb) -> cb.sum(root.get(dataField)));
		operations.put(Aggregable.Operation.MIN, (dataField, root, cb) -> cb.min(root.get(dataField)));
		operations.put(Aggregable.Operation.MAX, (dataField, root, cb) -> cb.max(root.get(dataField)));
		operations.put(Aggregable.Operation.AVG, (dataField, root, cb) -> cb.avg(root.get(dataField)));
	}
	...
	public Page<T> findAll(Class<T> entityClass, Pageable pageable, Projectable projectable, Class<? extends AbstractSpecification<T>> specificationClass, Payload payload) {
		if (Projectable.hasProjection(projectable)) {
			...
			Aggregation countAggregation = this.aggregationTemplate(entityClass, "id", spec, Aggregable.Operation.COUNT);
