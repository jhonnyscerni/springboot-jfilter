...
public class LessThanPredicate extends AbstractPredicate {
	...
	public <T> Predicate toPredicate(Class<T> clazz, Root<T> root, CriteriaQuery<?> cq, CriteriaBuilder cb, Map<String, AttributePath> paths) {
		...
		if (dataFieldType == Long.class) return cb.lt(path.as(Number.class), (Number) this.getValue());
		if (dataFieldType == Integer.class) return cb.lt(path.as(Number.class), (Number) this.getValue());
		if (dataFieldType == Short.class) return cb.lt(path.as(Number.class), (Number) this.getValue());
		if (dataFieldType == BigDecimal.class) return cb.lt(path.as(Number.class), new BigDecimal(String.valueOf(this.getValue())));
		if (dataFieldType == BigInteger.class) return cb.lt(path.as(Number.class), new BigInteger(String.valueOf(this.getValue())));
